...
def check_task_status(client, name, container_definition, task_options={}):
    ...
    saved_definition = client.describe_task_definition(taskDefinition=latest_task_arn)['taskDefinition']
    saved_containers = saved_definition['containerDefinitions']
    changed = regularization_task(saved_containers) != regularization_task(container_definition)
    if changed:
        return {'changed': True, 'arn': latest_task_arn}
    for key, val in task_options.items():
        if saved_definition[key] != val:
            return {'changed': True, 'arn': latest_task_arn}
    return {'changed': False, 'arn': latest_task_arn}
